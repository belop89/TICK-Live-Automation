...
// Använder nu JUCEs inbyggda och säkra SpinLock under huven istället för en CPU-brännande while-loop
class spin_lock
{
public:
    void lock() noexcept { lock_.enter(); }
    void unlock() noexcept { lock_.exit(); }
    bool try_lock() noexcept { return lock_.tryEnter(); }
...
private:
    ...
    void updatePrecalculatedSamples();
