...
struct TickUtils
{
    ...
    static std::unique_ptr<juce::ZipFile> getFactorySamples()
    {
        // Avoid std::make_unique here to prevent template instantiation issues with incomplete/complex JUCE types.
        return std::unique_ptr<juce::ZipFile>(
            new juce::ZipFile(
                new juce::MemoryInputStream (BinaryData::factory_samples_zip, BinaryData::factory_samples_zipSize, false),
                true));
    }
    ...
    static String generateTimecodeDisplay (const AudioPlayHead::PositionInfo& pos)
    {
        ...
        // FIX: Tvinga flyttalsberäkning (4.0) för att förhindra heltals-trunkering vid udda taktarter
        const double quarterNotesPerBar = (numerator * 4.0 / denominator);
